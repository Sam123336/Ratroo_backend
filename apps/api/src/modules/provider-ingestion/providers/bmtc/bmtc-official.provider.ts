...
export const BMTC_OFFICIAL_CONFIG: ProviderConfig = {
  ...
  endpoints: [
    {
      name: "Service Types",
      url: `${BMTC_BASE_URL}/GetAllServiceTypes`,
      method: "POST",
      format: "JSON",
    },
    {
      name: "Route List",
      url: `${BMTC_BASE_URL}/GetAllRouteList`,
      method: "POST",
      format: "JSON",
    },
    {
      name: "Route Search",
      url: `${BMTC_BASE_URL}/SearchRoute_v2`,
      method: "POST",
      format: "JSON",
    },
    {
      name: "Route Details",
      url: `${BMTC_BASE_URL}/SearchByRouteDetails_v4`,
      method: "POST",
      format: "JSON",
    },
    {
      name: "Timetable",
      url: `${BMTC_BASE_URL}/GetTimetableByRouteid_v3`,
      method: "POST",
      format: "JSON",
    },
  ],
...
/**
 * Where the client caches raw responses.
 *
 * Vercel mounts the deployed function bundle read-only at `/var/task`; only
 * the system temp directory is writable. The provider is instantiated while
 * Nest boots, so trying to create `.bmtc-cache` under `process.cwd()` crashes
 * every API route before it can serve a response.
 */
export function resolveBmtcCacheDir(
  env: Partial<
    Pick<NodeJS.ProcessEnv, "BMTC_CACHE_DIR" | "VERCEL">
  > = process.env,
  cwd = process.cwd(),
  temporaryDirectory = tmpdir(),
): string {
  if (env.BMTC_CACHE_DIR) return env.BMTC_CACHE_DIR;
  return env.VERCEL
    ? join(temporaryDirectory, "ratroo-bmtc-cache")
    : join(cwd, ".bmtc-cache");
}

const CACHE_DIR = resolveBmtcCacheDir();
...
export class BmtcFetcher implements IFetcher {
  ...
  constructor(
    cacheDir = CACHE_DIR,
    minIntervalMs = Number(process.env.BMTC_INTERVAL_MS ?? 1000),
  ) {
...
export class BmtcValidator implements IValidator<BmtcRouteRecord> {
  async validate(
    ...
    if (!records.length) {
      return {
        isValid: false,
        errors: ["No routes crawled from BMTC."],
        warnings,
      };
    }
    ...
    if (failed)
      warnings.push(`${failed}/${records.length} routes failed to crawl.`);
...
export class BmtcMapper implements IMapper<BmtcRouteRecord> {
  async map(
    ...
    for (const record of records) {
      ...
      routePatterns.push(
        toRoutePattern(record.route, record.stops, routeClass),
      );
        ...
        const mapped = timetableTrips(record.timetable)
          .map((trip, i) =>
            toTrip(
              ...
              trip,
              i,
              classByType.get(trip.servicetypeid ?? -1) ?? "UNKNOWN",
            ),
          )
...
@Injectable()
export class BmtcOfficialProvider extends BaseProviderAdapter<
  BmtcDiscoveryItem,
  BmtcRouteRecord
> {
  ...
  ): AsyncIterable<BmtcDiscoveryItem> {
   ...
   */
  async fetch(
    item: BmtcDiscoveryItem,
    context: ProviderRunContext,
  ): Promise<RawProviderResponse> {
    const client = this.fetcher.client;
    const { route, serviceTypes } = item;
    const record: BmtcRouteRecord = {
      route,
      serviceTypes,
      stops: [],
      timetable: null,
